...
public record UtilsService() {
  ...
  public Mono<List<Map<String, Object>>> createObjects(TypeDto type, Flux<Map<String, Object>> fluxObjects,
    ...
    return fluxObjects.flatMap(object -> mapRequiredFieldsForSaveActions(Mono.just(type), Mono.just(object), true,
        ...
        .flatMap(objects -> getTypePrimaryKey(Mono.just(type)).flatMapMany(
            primaryKey -> mongoTemplate.insertAll(Mono.just(objects),
                type.getName()).map(object -> mapIdFieldForApi(object, primaryKey))).collectList())
        .switchIfEmpty(Mono.error(new CreateObjectsFailed(type.getName())));
  }
  ...
  public Mono<Triplet<Map<String, Object>, String, String>> mapRequiredFieldsForSaveActions(Mono<TypeDto> monoType,
    ...
    Mono<Map<String, Object>> validated =
        monoType.flatMap(type -> monoObject.flatMap(
            object -> validateObject(Flux.fromIterable(type.getBackendKeyDefinitions()), object, typeDao,
                mongoTemplate)));
  ...
  public Mono<Triplet<Map<String, Object>, String, String>> mapRequiredFieldsForSaveActions(Mono<TypeDto> monoType,
    ...
    Mono<Map<String, Object>> validated =
        monoType.flatMap(type -> monoObject.flatMap(
            object -> validateObject(Flux.fromIterable(type.getBackendKeyDefinitions()), object, typeService,
                mongoTemplate)));
    return validated.flatMap(object -> Mono.zip(getTypePrimaryKey(monoType), monoTypeName).map(tuple -> {
      var document = mapIdFieldForEntity(object, tuple.getT1(), generateId);
      return new Triplet<>(document, tuple.getT1(), tuple.getT2());
  ...
  public Mono<String> getTypePrimaryKey(Mono<TypeDto> monoType) {
    return monoType.map(TypeDto::getBackendKeyDefinitions)
        .map(definitions ->
            definitions.stream().filter(definition -> definition instanceof PrimitiveBackendDefinitionDto)
                .map(definition -> (PrimitiveBackendDefinitionDto) definition)
                .filter(key -> BackendKeyType.PRIMARYKEY.equals(key.getType())).map(
        ...
        if (BackendKeyType.PRIMARYKEY.equals(Objects.requireNonNull(referencedDefinition).getType())) {
          ...
          referenceObject =
              mongoTemplate.findOne(Query.query(
                      Criteria.where(referencedDefinition.getKey()).is(object.get(referencedDefinition.getKey()))),
                  BasicDBObject.class, referencedType.getName());
        }
